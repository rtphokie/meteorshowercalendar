...
def sldate(t, target_longitude):
    sun, earth = eph['sun'], eph['earth']
    astrometric = eph['Earth'].at(t).observe(eph['Sun'])
    lat, lon, distance = astrometric.frame_latlon(ecliptic_frame)
    i = min(range(len(lon.degrees)), key=lambda i: abs(lon.degrees[i] - target_longitude))
    dt = t[i].utc_datetime()
    # print(dt, lon.degrees[i])
    return dt
...
class MyTestCase(unittest.TestCase):
    ...
    def test_meteor_table(self):
        ...
        for key, data in foo['showers'].items():
            ...
            for activity in data['activity']:
                if activity['year'] == 'generic':
                    ...
                    # largest outburst
                    if 'zhr' in activity and ( shower['outburst_year_largest'] is None or shower['outburst_zhr_largest'] < activity[ 'zhr']):
                        measurement='largest'
        ...
        print(df[['name',  'zhr_low', 'zhr', 'variable', 'date_peak',  'outburst_zhr_largest', 'outburst_date_largest', 'outburst_zhr_last', 'outburst_date_last']])
        # print(df)
